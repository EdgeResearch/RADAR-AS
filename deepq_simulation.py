...
class DeepQLearning:
    ...
    #HO AGGIUNGO IL LOG DELLE AZIONI COMPIUTE DAL Super-agent
    # deepq_simulation.py
    def predict_sa_action(self, env, observation, log_manager=None):
        observation = self.check_tuple(observation)
        encoded_reshaped = observation.reshape([1, observation.shape[0]])
        predicted = self.target_model.predict(encoded_reshaped, verbose=0).flatten()

        action = int(np.argmax(predicted))
        ...
        virality = float(new_observation[0])

        if log_manager is not None:
            current_tick = int(env.netlogo.get_current_tick())
            log_manager.insert_line(
                f"[SA_ACTION] tick={current_tick} action={action} ({action_name}) "
                f"virality={virality:.4f} reward={float(reward):.4f}"
            )
